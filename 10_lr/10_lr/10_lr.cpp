// 10_lr.cpp: определяет точку входа для консольного приложения.
//
...
class APDExc { //класс исключений
public:
	class BelowZero { // отрицательное число
	public: BelowZero(string asd) {
		error = asd;
	}
			string GetError() {
				return this->error;
...
template <typename a, typename b>
APDevelopment<a, b>::~APDevelopment() {
	delete[] filter;
}
...
ApplicationDevelopment<a, b>::ApplicationDevelopment(const a * nameVal, const a * versionVal, b costVal, const a * platformVal, const a * languageVal) {
	...
	language = nullptr;
	try {
		if (costVal < 0) throw APDExc::BelowZero("Стоимоть меньше 0");
	...
	catch (APDExc::BelowZero e) {
		cout << "Ошибка:" << e.GetError() << endl;
		cout << "Объект не будет создан" << endl;
	}
}
...
template <typename a, typename b>
ApplicationDevelopment<a, b>::~ApplicationDevelopment() {
	if (name != nullptr && version != nullptr && platform != nullptr && language != nullptr)
	delete[] name;
	delete[] version;
	delete[] platform;
	delete[] language;
...
void ApplicationDevelopment<a, b>::setversion(char * newversion) {
	...
	strcpy_s(this->name, strlen(newversion) + 1, newversion);
}

template <typename a, typename b>
void ApplicationDevelopment<a, b>::setcost(int newcost) {
	try {
		if (newcost < 0) throw APDExc::BelowZero("Стоимость - отрицательное число");
		this->cost = newcost;
	}
	catch (APDExc::BelowZero e) {
		cout << "Ошибка: " << e.GetError() << endl;
		cout << "Изменение не было выполнено" << endl;
	}
}
